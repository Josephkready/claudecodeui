...
test('getSessionTokenUsage returns an unsupported payload for Cursor sessions', async () => {
  const result = await getSessionTokenUsage('cursor-session-id', {
    getSessionById: () => ({ provider: 'cursor' }),
    getClaudeUsage: async () => {
      throw new Error('claude path should not be reached for cursor sessions');
    },
    resolveCodexSessionsDir: () => '/never-touched',
  });
